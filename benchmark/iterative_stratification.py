...
def iterative_train_test_split(X, y, test_size, random_state=None, shuffle=None):
...
class IterativeStratification(_BaseKFold):
    ...
    def __init__(self, n_splits=3, order=1, sample_distribution_per_fold=None, random_state=None, shuffle=False):
        self.order = order
        super(
...
def main():
    ...
    for itra in range(10):
        kfolds = IterativeStratification(n_splits=4, order=1, random_state=itra, shuffle=True)
        all_folds = []
        all_folds_labels = []
        class_std = []
        for train_idx, test_idx in kfolds.split(samples, encoded_labels):
            counts = {}
            print("TRAIN:", train_idx, "TEST:", test_idx)
            training_samples, training_labels = samples[[train_idx]], encoded_labels[tuple([train_idx])]
            test_samples, test_labels = samples[[test_idx]], encoded_labels[tuple([test_idx])]
            print(training_samples, training_labels)
            print(test_samples, test_labels)

            training_samples, training_labels, dev_samples, dev_labels = iterative_train_test_split(training_samples,
                                                                                                    training_labels,
                                                                                                    test_size=0.5,
                                                                                                    random_state=itra,
                                                                                                    shuffle=True)

            all_folds.append((training_samples, dev_samples, test_samples))
            all_folds_labels.append((training_labels, dev_labels, test_labels))

            print(f"train: {len(training_samples)} ({len(training_samples)/len(samples):.2f})\n"
                  f"dev: {len(dev_samples)} ({len(dev_samples)/len(samples):.2f})\n"
                  f"test: {len(test_samples)} ({len(test_samples)/len(samples):.2f})")

            # Get counts for each class
            counts["train_counts"] = Counter(str(combination) for row in
                                             get_combination_wise_output_matrix(training_labels, order=1)
                                             for combination in row)
            counts["val_counts"] = Counter(str(combination) for row in
                                           get_combination_wise_output_matrix(dev_labels, order=1)
                                           for combination in row)
            counts["test_counts"] = Counter(str(combination) for row in
                                            get_combination_wise_output_matrix(test_labels, order=1)
                                            for combination in row)
            ...
            print(f"this fold's st.dev: {fold_std}")
            class_std.append(fold_std)

        best_fold_indx = np.argmin(class_std)
        print(f"fold with lowest std: {best_fold_indx} - {class_std[best_fold_indx]}")
        print(all_folds[best_fold_indx])
        print(all_folds_labels[best_fold_indx])
        if fold_std < best_cls_std:
            best_cls_std = fold_std
            best_seed = itra

    print(f"best seed: {best_seed}; lowest cls dis std: {best_cls_std}")
