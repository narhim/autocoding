...
def build_icd10_hierarchy(
    ...
    for chapter_elem in xml_root.ICD10CM_tabular.chapter:
        ...
        for section_elem in chapter_elem.section:
            ...
            G.add_node(section,section=section_elem['id'])
            G.add_edge(chapter,section)
            try: #Y36
                for diag_elem in section_elem.diag:
                    diag = diag_elem.desc.cdata
                    G.add_node(diag_elem.name.cdata,subsection=diag)
                    G.add_edge(section,diag_elem.name.cdata)
                    #try: #Y36.0 #Here is where it becomes cyclic, don't know why.
                    #    for diag_diag_elem in diag_elem.diag:
                    #        diag_diag = diag_diag_elem.desc.cdata
                    #        G.add_node(diag_diag_elem.name.cdata,subsection=diag_diag)
                    #        G.add_edge(diag,diag_diag_elem.name.cdata)
                    #        #print(nx.find_cycle(G, source=root_name, orientation=None))
                    #        try: #Y36.00
                    #            for diag_diag_diag_elem in diag_diag_elem.diag:
                    #                diag_diag_diag = diag_diag_diag_elem.desc.cdata
                    #                G.add_node(diag_diag_diag,subsubsubsection=diag_diag_diag_elem.name.cdata)
                    #                G.add_edge(diag_diag,diag_diag_diag)
                    #                try: #Y36.000
                    #                    for diag_diag_diag_diag_elem in diag_diag_diag_elem.diag:
                    #                        diag_diag_diag_diag = diag_diag_diag_diag_elem.desc.cdata
                    #                        G.add_node(diag_diag_diag_diag,subsubsubsubsection=diag_diag_diag_diag_elem.name.cdata)
                    #                        G.add_edge(diag_diag_diag,diag_diag_diag_diag)
                    #                except:
                    #                    pass
                    #        except:
                    #            pass
